/**
 * Endpoint Vercel: POST /api/extract-monitoreo
 * Autocontenido (sin imports locales) para evitar FUNCTION_INVOCATION_FAILED
 * por bundling ESM de módulos relativos en proyectos "type": "module".
 */
import type { VercelRequest, VercelResponse } from '@vercel/node'

type ExtractedMonitoreoParam = {
  parametro: string
...
type ExtractedMonitoreoInforme = {
...
Reglas:
- Un informe puede tener VARIOS puntos. Crea un objeto en "muestreos" por cada punto.
- Extrae TODOS los analitos con valor numérico.
- resultado: número; si viene "<0.01" usa 0.01 y anota en observaciones.
- No inventes cifras.
- Cliente Alicón / Cementos Progreso → unidadNegocio "Alicón", plantaSede "Alicon".
...
async function extractMonitoreoFromText(input: {
  text: string
  fileName?: string
  apiKey: string
    ...
    return res.status(500).json({
      error:
        err instanceof Error
          ? err.message
          : 'Error interno al extraer el informe',
    })
  }
}
