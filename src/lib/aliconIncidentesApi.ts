import {
  ALICON_INCIDENTE_SEDE,
  ALICON_INCIDENTE_UNIDAD,
  anioMesFromFecha,
  buildFecha,
  mesTextoFromFecha,
  parseIntSafe,
...
export async function loadAliconIncidentes(): Promise<AgroIncidenteRecord[]> {
  const { data, error } = await supabase
    .from('incidentes_ambientales')
    .select(SELECT_COLS)
    .eq('planta_sede', ALICON_INCIDENTE_SEDE)
...
export async function saveAliconIncidentesMonth(
  year: number,
  ...
  const payload = rows.map((row) => {
    ...
    return {
      ...
      unidad_negocio: ALICON_INCIDENTE_UNIDAD,
      planta_sede: ALICON_INCIDENTE_SEDE,
