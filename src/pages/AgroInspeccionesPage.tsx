...
import {
  ...
  saveAgroInspeccionesMonth,
} from '../lib/agroInspeccionesApi'
...
export function AgroInspeccionesPage() {
  ...
  async function reload() {
    ...
    setLoadError(null)
    try {
      const data = await loadAgroInspecciones()
      setRecords(data)
    } catch (err) {
      setLoadError(
  ...
  useEffect(() => {
    ...
    setFormRows(rows.length ? rows : [emptyInspeccionRow(year, month)])
  }, [records, year, month])
  ...
  function addRow() {
    setFormRows((rows) => [...rows, emptyInspeccionRow(year, month)])
  }

  function removeRow(localId: string) {
    setFormRows((rows) => {
      const next = rows.filter((r) => r.localId !== localId)
      return next.length ? next : [emptyInspeccionRow(year, month)]
    })
  }
  ...
  async function persist() {
    ...
    try {
      ...
      const saved = await saveAgroInspeccionesMonth(year, month, toSave)
      setRecords((prev) => {
  ...
  return (
    <div className="entry-page hc-entry agro-inspecciones-page">
      <div className="page-header entry-header">
        <div>
          ...
          <p>
            Réplica de <strong>Ejecuciones inspecciones</strong> (solo
            Agroprogreso). Captura por mes: día, sede, resultado, hallazgos y
            evidencias.
          </p>
        </div>
      ...
      <form className="entry-form" onSubmit={handleSubmit}>
        <div className="hc-month-bar">
          <div className="agro-agua-period">
            <label htmlFor="agro-insp-year">Año</label>
            <select
              id="agro-insp-year"
              value={year}
        ...
        <section className="entry-section">
          <div className="entry-section-head alicon-sheet-head">
            <div>
              ...
              <p>
                Completa día, sede, resultado (0–100), hallazgos, riesgo y
                observaciones. La semana y fecha se calculan al guardar.
              </p>
            </div>
              ...
              <article key={row.localId} className="agro-insp-card">
                ...
                <div className="agro-insp-grid">
                  ...
                  <label>
                    ...
                    >
                      {AGRO_INSPECCION_SEDES.map((s) => (
                        <option key={s} value={s}>
                          {s}
                        </option>
                      ))}
                      {row.plantaSede &&
                      !(AGRO_INSPECCION_SEDES as readonly string[]).includes(
                        row.plantaSede,
                      ) ? (
      ...
      <section className="entry-section agro-records-section">
        <div className="entry-section-head">
          ...
          <p>
            ...
            Agroprogreso · fuente Ejecuciones inspecciones.
          </p>
        </div>
