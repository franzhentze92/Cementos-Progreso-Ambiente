...
import {
  ALICON_INCIDENTE_INSTRUMENTOS,
  ALICON_INCIDENTE_SEDES,
  INCIDENTE_ESTADOS,
  MONITORING_MONTHS,
  countAbiertosMes,
  emptyAliconIncidenteRow,
  formRowsFromRecords,
  formatPctFromValor,
  monthFromFecha,
  monthHasIncidentes,
...
import {
  ...
  saveAliconIncidentesMonth,
} from '../lib/aliconIncidentesApi'
...
export function AliconIncidentesPage() {
  ...
  async function reload() {
    ...
    setLoadError(null)
    try {
      const data = await loadAliconIncidentes()
      setRecords(data)
    } catch (err) {
      setLoadError(
  ...
  useEffect(() => {
    ...
    setFormRows(rows.length ? rows : [emptyAliconIncidenteRow(year, month)])
  }, [records, year, month])
  ...
  function addRow() {
    setFormRows((rows) => [...rows, emptyAliconIncidenteRow(year, month)])
  }

  function removeRow(localId: string) {
    setFormRows((rows) => {
      const next = rows.filter((r) => r.localId !== localId)
      return next.length ? next : [emptyAliconIncidenteRow(year, month)]
    })
  }
  ...
  async function persist() {
    ...
    try {
      ...
      const saved = await saveAliconIncidentesMonth(year, month, toSave)
      setRecords((prev) => {
  ...
  if (loading) {
    return (
      <div className="entry-page hc-entry hc-loading">
        ...
        <p>Cargando incidentes ambientales Planta Alicón…</p>
      </div>
    )
  }

  return (
    <div className="entry-page hc-entry alicon-incidentes-page">
      <div className="page-header entry-header">
        <div>
          ...
          <p>
            Réplica de la hoja <strong>Incidentes</strong> (solo Planta Alicón).
              ...
              <article key={row.localId} className="agro-insp-card">
                ...
                <div className="agro-insp-grid">
                  ...
                  <label>
                    ...
                    >
                      {ALICON_INCIDENTE_SEDES.map((s) => (
                        <option key={s} value={s}>
                          {s}
                        </option>
                      ))}
                      {row.plantaSede &&
                      !(ALICON_INCIDENTE_SEDES as readonly string[]).includes(
                        row.plantaSede,
                      ) ? (
                  ...
                  <label>
                    ...
                    >
                      {ALICON_INCIDENTE_INSTRUMENTOS.map((s) => (
                        <option key={s} value={s}>
                          {s}
                        </option>
                      ))}
                      {row.instrumento &&
                      !(
                        ALICON_INCIDENTE_INSTRUMENTOS as readonly string[]
      ...
      <section className="entry-section agro-records-section">
        <div className="entry-section-head">
          ...
          <p>
            {filteredRecords.length} de {records.length} incidentes Alicón
