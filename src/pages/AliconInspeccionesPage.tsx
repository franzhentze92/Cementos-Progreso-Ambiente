...
import {
  ACCION_INMEDIATA,
  ALICON_INSPECCION_SEDES,
  MONITORING_MONTHS,
  NIVELES_RIESGO,
  avgResultadoForMonth,
  emptyAliconInspeccionRow,
  formRowsFromRecords,
  formatNum,
  monthFromFecha,
  monthHasInspections,
...
export function AliconInspeccionesPage() {
  ...
  async function reload() {
    ...
    setLoadError(null)
    try {
      const data = await loadAliconInspecciones()
      setRecords(data)
    } catch (err) {
      setLoadError(
  ...
  useEffect(() => {
    ...
    setFormRows(rows.length ? rows : [emptyAliconInspeccionRow(year, month)])
  }, [records, year, month])
  ...
  function addRow() {
    setFormRows((rows) => [...rows, emptyAliconInspeccionRow(year, month)])
  }

  function removeRow(localId: string) {
    setFormRows((rows) => {
      const next = rows.filter((r) => r.localId !== localId)
      return next.length ? next : [emptyAliconInspeccionRow(year, month)]
    })
  }
  ...
  async function persist() {
    ...
    try {
      ...
      const saved = await saveAliconInspeccionesMonth(year, month, toSave)
      setRecords((prev) => {
      ...
      <div className="page-header entry-header">
        <div>
          ...
          <p>
            ...
            sede Alicon). Captura por mes: día, resultado, hallazgos y
            evidencias.
          </p>
        </div>
              ...
              <article key={row.localId} className="agro-insp-card">
                ...
                <div className="agro-insp-grid">
                  ...
                  <label>
                    ...
                    >
                      {ALICON_INSPECCION_SEDES.map((s) => (
                        <option key={s} value={s}>
                          {s}
                        </option>
                      ))}
                      {row.plantaSede &&
                      !(ALICON_INSPECCION_SEDES as readonly string[]).includes(
                        row.plantaSede,
                      ) ? (
      ...
      <section className="entry-section agro-records-section">
        <div className="entry-section-head">
          ...
          <p>
            ...
            Planta Alicón · fuente Ejecuciones inspecciones.
          </p>
        </div>
