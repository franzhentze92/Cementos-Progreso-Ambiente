...
import {
  buildAliconInspeccionReport,
  type AliconInspeccionReport,
} from '../data/aliconInspeccionesReport'
import { formatNum } from '../data/aliconInspecciones'
import { loadAliconInspecciones } from '../lib/aliconInspeccionesApi'
...
export function AliconInspeccionesReportPage() {
  const [report, setReport] = useState<AliconInspeccionReport | null>(null)
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      setError(null)
      try {
        const records = await loadAliconInspecciones()
        if (cancelled) return
        setRawCount(records.length)
        const built = buildAliconInspeccionReport(records, year)
  ...
  if (loading) {
    return (
      <div className="carbon-page hc-loading">
        ...
        <p>Cargando inspección ambiental Planta Alicón…</p>
      </div>
    )
  }
    ...
    <div className="carbon-page agro-inspecciones-report alicon-inspecciones-report">
      <div className="page-header carbon-header">
        <div>
          ...
          <p>
            Datos reales · hoja Ejecuciones inspecciones (sede Alicon) ·{' '}
            {meta.periodLabel}
          </p>
        </div>
                  ...
                  <tr key={`${row.fecha}-${row.sede}-${i}`}>
                    ...
                    <td>
                      <InspeccionAbrirLink
                        ...
                        fecha={row.fecha}
                        plantaSede={row.sede}
                        unidadNegocio="Cementos Progreso"
                      />
                    </td>
                  </tr>
                ))
      ...
      <p className="agro-report-footnote">
        ...
        {totals.totalHallazgos
          ...
          : ''}
        . Solo datos reales (Planta Alicón).
      </p>
    </div>
  )
}
