...
import {
  buildDescargaBarcosInspeccionReport,
  type DescargaBarcosInspeccionReport,
} from '../data/descargaBarcosInspeccionesReport'
import { formatNum } from '../data/descargaBarcosInspecciones'
import { loadDescargaBarcosInspecciones } from '../lib/descargaBarcosInspeccionesApi'
...
export function DescargaBarcosInspeccionesReportPage() {
  const [report, setReport] = useState<DescargaBarcosInspeccionReport | null>(null)
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      setError(null)
      try {
        const records = await loadDescargaBarcosInspecciones()
        if (cancelled) return
        setRawCount(records.length)
        const built = buildDescargaBarcosInspeccionReport(records, year)
  ...
  return (
    <div className="carbon-page agro-inspecciones-report alicon-inspecciones-report">
      <div className="page-header carbon-header">
        <div>
          ...
          <h1>Inspecciones</h1>
          <p>
            Datos reales · hoja Ejecuciones inspecciones (sede Descarga Barcos) ·{' '}
            {meta.periodLabel}
          </p>
        </div>
        <div className="carbon-header-meta">
          ...
          <div>
            <span>Periodo</span>
            <label className="agro-year-filter" htmlFor="alicon-insp-report-year">
              <select
                id="alicon-insp-report-year"
          ...
          <table className="carbon-table">
            ...
            <tbody>
              {report.detailRows.length === 0 ? (
                <tr>
                  <td colSpan={11}>Sin inspecciones en el periodo.</td>
                </tr>
              ) : (
                report.detailRows.map((row, i) => (
                  <tr key={`${row.fecha}-${row.sede}-${row.material}-${i}`}>
                    <td>{row.fecha}</td>
                    <td>{row.mes}</td>
                    <td>{row.material}</td>
                    <td>{row.sede}</td>
      ...
      <p className="agro-report-footnote">
        ...
        {totals.totalHallazgos
          ...
          : ''}
        . Solo datos reales (Descarga Barcos).
      </p>
    </div>
  )
}
