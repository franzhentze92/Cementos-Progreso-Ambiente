...
/** SVG icons (white) by facility type */
...
function createTypeIcon(type: LocationType, active: boolean) {
...
function MapFlyTo({ site }: { site: SiteLocation | null }) {
  const map = useMap()
  ...
  return (
    <div className="map-page">
      <div className="page-header">
        <h1>Mapa</h1>
        <p>
          {SITES.length} ubicaciones públicas de plantas, concreto, centros y
          oficinas de Cementos Progreso en la región.
        </p>
      </div>
        ...
        <div className="map-frame">
          ...
          >
            ...
            <TileLayer
              ...
              opacity={0.9}
            />
            {!selected && <FitFilteredBounds key={fitKey} sites={filtered} />}
            <MapFlyTo site={selected} />
            {filtered.map((site) => (
              <Marker
                key={site.id}
                position={[site.lat, site.lng]}
                icon={createTypeIcon(site.type, site.id === selectedId)}
                eventHandlers={{
          ...
          {selected ? (
            ...
            <>
              ...
              <ul className="map-site-list">
                {filtered.map((site) => (
                  <li key={site.id}>
                    <button type="button" onClick={() => setSelectedId(site.id)}>
                      <i
                        className="map-list-icon"
                        style={{ background: TYPE_COLORS[site.type] }}
                        dangerouslySetInnerHTML={{ __html: TYPE_SVGS[site.type] }}
                      />
                      <span>
                        <strong>{site.name}</strong>
                        <small>
